...
class GalleryApp {
  ...
  planeGeometry: any; medias: MediaItem[] = [];
  mediasImages: any[] = [];
  ...
  constructor(
    container: HTMLElement,
    scrollTargetRef: { current: number },
    { items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree' }: any = {}
  ) {
    this.container = container;
    this.scrollTargetRef = scrollTargetRef;
  ...
  update() {
    // Drive scroll from external scroll progress
    this.scroll.target = this.scrollTargetRef.current;
...
interface CircularGalleryProps {
  ...
  borderRadius?: number;
  font?: string;
  scrollOffset?: number; // external scroll-driven offset
}
...
export default function CircularGallery({
  ...
  // Update scroll target whenever scrollOffset changes
  scrollTargetRef.current = scrollOffset;

  useEffect(() => {
    if (!containerRef.current) return;
    const app = new GalleryApp(containerRef.current, scrollTargetRef, {
